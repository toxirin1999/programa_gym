...
Para ejecutar:
python manage.py shell < limpieza_datos.py
"""
...
try:
    # from entrenos.models import SerieRealizada, EntrenoRealizado, PlanPersonalizado, RutinaEjercicio
...
# def limpiar_rutina_ejercicio():
#     """Limpia valores no numéricos en RutinaEjercicio.peso_kg"""
#     logger.info("Iniciando limpieza de RutinaEjercicio.peso_kg")
#
#     # Obtener todas las asignaciones
#     asignaciones = RutinaEjercicio.objects.all()
#     total = asignaciones.count()
#     corregidas = 0
#     problematicas = 0
#
#     logger.info(f"Total de asignaciones a procesar: {total}")
#
...
#                 try:
#                     Decimal(str(asignacion.peso_kg))
#                     # Si llega aquí, el valor es válido
#                 except (InvalidOperation, ValueError, TypeError):
#                     # Valor inválido, intentar corregir
#                     problematicas += 1
#                     valor_original = asignacion.peso_kg
#
#                     # Intentar limpiar y convertir
#                     try:
#                         # Convertir a string y reemplazar coma por punto
...
#                             # Si quedó algo después de limpiar
#                             nuevo_valor = Decimal(valor_limpio)
#                             asignacion.peso_kg = nuevo_valor
#                             asignacion.save()
#                             corregidas += 1
#                             logger.info(
#                                 f"Asignación ID {asignacion.id}: Valor corregido de '{valor_original}' a '{nuevo_valor}'")
#                         else:
#                             # Si no quedó nada válido, establecer a 0
#                             asignacion.peso_kg = Decimal('0')
#                             asignacion.save()
#                             corregidas += 1
#                             logger.info(
#                                 f"Asignación ID {asignacion.id}: Valor inválido '{valor_original}' establecido a 0")
#                     except Exception as e:
#                         # Si todo falla, establecer a 0
#                         logger.warning(f"Asignación ID {asignacion.id}: Error al corregir '{valor_original}': {str(e)}")
#                         asignacion.peso_kg = Decimal('0')
#                         asignacion.save()
#                         corregidas += 1
#                         logger.info(f"Asignación ID {asignacion.id}: Valor problemático establecido a 0")
#         except Exception as e:
#             logger.error(f"Error al procesar asignación ID {asignacion.id}: {str(e)}")
#
#     logger.info(f"Limpieza de RutinaEjercicio.peso_kg completada:")
#     logger.info(f"- Total asignaciones procesadas: {total}")
#     logger.info(f"- Asignaciones problemáticas encontradas: {problematicas}")
#     logger.info(f"- Asignaciones corregidas: {corregidas}")
#
#     return problematicas, corregidas
...
def ejecutar_limpieza():
    ...
    try:
        ...
        logger.info(
            f"PlanPersonalizado.peso_objetivo: {planes_problematicos} problemas, {planes_corregidos} corregidos")
