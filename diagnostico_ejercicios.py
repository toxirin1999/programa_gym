...
cliente_id = 1  # Ajusta si es necesario
...
for entreno in entrenos:
    ...
    for ej in ejercicios:
        ...
        try:
            ...
            print(f" ⚠️  Ignorado: {nombre} (peso={peso}, reps={reps_raw}) - Formato inválido")
            continue

        if reps > 12:
            print(f" ❌ {nombre} - Reps {reps} > 12 → descartado para 1RM")
        elif peso_val == 0 and nombre.lower() != "dominadas":
            print(f" ❌ {nombre} - Peso 0 → descartado (excepto dominadas)")
        elif nombre.strip() == "":
            print(f" ❌ Sin nombre → ignorado")
        else:
