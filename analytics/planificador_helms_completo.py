...
class PlanificadorHelms:
    ...
    def _calcular_peso_trabajo(self, nombre_ejercicio: str, repeticiones_str: str, rpe_objetivo: int) -> float:
        """
        Estima el peso de trabajo. VERSIÓN CORREGIDA: Normaliza el nombre del ejercicio.
        """
        # 1. Normalizar el nombre del ejercicio a minúsculas antes de buscar
        nombre_normalizado = nombre_ejercicio.strip().lower()

        # 2. Obtener el 1RM usando la clave normalizada
        # self.perfil.maximos_actuales ahora contiene los RMs actualizados y normalizados desde la vista
        one_rm_estimado = self.perfil.maximos_actuales.get(nombre_normalizado, 60.0)

        # 3. El resto de la lógica de cálculo se mantiene igual
        try:
            reps_objetivo = int(repeticiones_str.split('-')[-1])
        except:
            reps_objetivo = 8

        try:
            peso_rpe_10 = one_rm_estimado / (1 + (reps_objetivo / 30))
            reduccion_por_rpe = (10 - rpe_objetivo) * 0.03
            peso_ajustado = peso_rpe_10 * (1 - reduccion_por_rpe)
            return round(peso_ajustado / 2.5) * 2.5
